...
void draw_select_project() {
  ...
  for( Project project : projects ) {
    ...
    project.ui.selected = ( active_project == project );
...
void draw_config() {
  ...
  if( active_cam != null ) {
    pushMatrix();
      ...
      //   camera
      float scalar      = 1.0;
      float fixed_width = 2*horiz_margin + 120 + border + textWidth( active_cam.getName() );
...
void draw_title( String title ) {
  ...
  frame.setTitle( project_name + " | " + title + " | Timelapse Capture | 0.0.1" );
  if( title_x < -textWidth( title ) ) title_x = width; title_x -= 1;
}
...
void draw_text( String what ) {
  ...
  fill(   0 ); text( what, x + 1, y + 1 );
  fill( 255 ); text( what, x,     y );
}
...
// ---------------------------------------------------------------------------------------- //
void draw_ui_topright    ( Clickable ui, int x, int y ) { draw_ui_topleft( ui, (int)(width - ui.w - x), y ); }
void draw_ui_bottomleft  ( Clickable ui, int x, int y ) { draw_ui_topleft( ui, x, (int)(height - ui.h - y) ); }
void draw_ui_bottomright ( Clickable ui, int x, int y ) { draw_ui_topleft( ui, (int)(width - ui.w - x), (int)(height - ui.h - y) ); }
